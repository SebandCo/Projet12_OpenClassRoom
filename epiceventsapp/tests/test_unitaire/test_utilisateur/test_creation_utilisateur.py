...
def test_good_user(mock_request):
    user, message = user_creation(mock_request)
    expected_value_user = mock_request.form
...
def test_wrong_user_surname(mock_request):
    mock_request.form["surname"] = ""
    user, message = user_creation(mock_request)
    expected_value_message = "No Surname"
    assert user == ""
    assert expected_value_message in message

    mock_request.form["surname"] = "a" * (MAX_LEN_VALUE+1)
    user, message = user_creation(mock_request)
...
def test_wrong_user_name(mock_request):
...
def test_wrong_user_department(mock_request):
    mock_request.form["department"] = ""
    user, message = user_creation(mock_request)
...
def test_wrong_user_identifiant(mock_request):
    mock_request.form["identifiant"] = ""
    user, message = user_creation(mock_request)
    expected_value_message = "No Identifiant"
    assert user == ""
    assert expected_value_message in message

    mock_request.form["identifiant"] = "a" * (MAX_LEN_VALUE+1)
    user, message = user_creation(mock_request)
...
def test_wrong_user_password(mock_request):
    ...
    mock_request.form["password"] = "a" * (MAX_LEN_VALUE+1)
    user, message = user_creation(mock_request)
    expected_value_message = "Too Long Password"
    assert user == ""
    assert expected_value_message in message

    mock_request.form["password"] = "different_password"
    user, message = user_creation(mock_request)
